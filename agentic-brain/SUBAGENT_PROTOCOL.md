...
Before spawning a subagent for a feature, the main agent must extract the 'Affected files' and 'Key functions' lists from TASKS.md and include them in the subagent's isolated context.

## Purpose
To isolate large, feature-size tasks so the main agent (Kilo) never accumulates context across domains. This keeps every agent session focused, lean, and reproducible.
...
- The main agent does **not** read the next task. It outputs a short summary and waits for the user's "continue" or next instruction.