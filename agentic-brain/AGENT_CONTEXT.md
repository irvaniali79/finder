...
- Chunk text is stored in a SQLite DB (`.cache/chunks.db`) via the `ChunkStore` class; each chunk now has `tags`, `summary`, `importance` metadata extracted at ingestion time
